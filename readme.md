...
**Next:** optimizers (SGD → Adam), `Linear`/`Module` layers and training loops, then SIMD and CUDA backends behind the same kernel interface, and later softmax/cross-entropy, CNNs, and attention. See [ROADMAP.md](ROADMAP.md) for the full plan.
